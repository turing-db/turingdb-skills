...
-- Set a vector embedding (parentheses, not brackets — see note below):
MATCH (n:Person {name: 'Alice'}) SET n.emb = (1.2, 2.0, 0.0, 12.0)

-- SET also works by internal node ID:
MATCH (n) WHERE n = 1234 SET n.emb = (1.2, 2.0, 0.0, 12.0)
```
...
## DELETE Syntax (Remove Nodes and Edges)

DELETE follows a MATCH and removes the matched nodes and/or edges.
...
-- Delete an edge:
MATCH (n:Person {name: 'Alice'})-[e:KNOWS]->(m) DELETE e

-- Delete multiple bound entities:
MATCH (n:Person {name: 'Remy'})-[e]->(m:Person) DELETE n, e
```
...
| `MERGE_DATAPARTS` | Compact the current graph's accumulated DataParts into one |

## Property Indexes

A property index speeds up equality lookups (`WHERE n.prop = <value>`) — the optimizer rewrites them into an index lookup. Creating an index is a **write**, so do it inside a change and submit:
...
List indexes with `CALL db.showIndexes()`; remove one with `DROP INDEX age_index`.
